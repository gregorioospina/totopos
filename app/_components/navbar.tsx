...
const Navbar = (props: INavbar) => {
	...
	return (
		<div
			className={[
				"fixed z-50 top-5 left-5 right-5 shadow-lg p-1 h-10 rounded-lg transition-opacity duration-700 bg-background-2",
				hide ? "opacity-0" : "opacity-100",
			].join(" ")}>
			<Link href={"/"} className="h-8 absolute left-0 top-1 bottom-1 w-20">
			...
			<div className="ml-22 flex gap-x-4 pr-3 h-8 items-center justify-start overflow-auto">
				<Link href={"/rsvp"} className="text-foreground text-sm whitespace-nowrap">
					Diego & Maca
				</Link>
				<Link href={"/rsvp"} className="text-foreground text-sm whitespace-nowrap">
					RSVP
